class PoultryApp {
    ...
    async init() {
        ...
        try {
            ...
            // Initialize desktop auth enhancements if available (after auth is confirmed)
            if (window.desktopAuth) {
                window.desktopAuth.init();
            }

            // Check authentication first
            if (!window.auth.checkSession()) {
                await window.auth.showLoginScreen();
