class VaccinationManager {
            ...
            <div class="card mb-4">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <div class="btn-group btn-group-sm">
                        <button class="btn btn-outline-primary" onclick="window.vaccinationManager.showScheduleManager()">
                            <i class="fas fa-edit me-1"></i>Customize Schedule
                        </button>
                        <button class="btn btn-outline-secondary" onclick="window.vaccinationManager.resetToStandardSchedule()">
                ...
                <div class="card-body">
                    ${schedule.length === 0 ? `
                        <div class="text-center py-4">
                            ...
                            <button class="btn btn-primary me-2" onclick="window.vaccinationManager.showScheduleManager()">
                                <i class="fas fa-plus me-1"></i>Create Custom Schedule
                            </button>
                            <button class="btn btn-outline-primary" onclick="window.vaccinationManager.resetToStandardSchedule()">
    ...
    renderVaccinationRow(vaccination) {
        ...
        const vaccineName = vaccination.vaccineName || vaccination.vaccine || 'Unknown';
        const administrationMethod = vaccination.administrationMethod || vaccination.method || 'Unknown';
        const birdsTreated = vaccination.birdsTreated || vaccination.birdsCount || 'N/A';
        const veterinarian = vaccination.veterinarian || '-';
        
        return `
            <tr>
                <td>${this.formatDate(vaccination.date)}</td>
                <td><span class="badge bg-info">${vaccination.flockAge || 'N/A'}</span></td>
                <td><strong>${vaccineName}</strong></td>
                <td>
                    <span class="badge ${this.getMethodBadgeClass(administrationMethod)}">
                        ${administrationMethod ? administrationMethod.replace('_', ' ') : 'Unknown'}
                    </span>
                </td>
                <td>${birdsTreated}</td>
                <td>${veterinarian}</td>
                <td>
                    <div class="btn-group btn-group-sm">
                        <button class="btn btn-outline-info" onclick="window.vaccinationManager.viewVaccinationDetails(${vaccination.id})">
                            <i class="fas fa-eye"></i>
                        </button>
                        <button class="btn btn-outline-danger" onclick="window.vaccinationManager.deleteVaccination(${vaccination.id})">
    ...
    viewVaccinationDetails(vaccinationId) {
        console.log('Viewing vaccination details for ID:', vaccinationId);
        
        const vaccination = this.vaccinations.find(v => v.id === vaccinationId);
        if (!vaccination) {
            console.error('Vaccination not found with ID:', vaccinationId);
            return;
        }

        console.log('Found vaccination:', vaccination);

        try {
            // Safe method formatting - handle both old and new data formats
            const administrationMethod = vaccination.administrationMethod || vaccination.method || 'Unknown';
            const methodText = administrationMethod.replace('_', ' ');
            const vaccineName = vaccination.vaccineName || vaccination.vaccine || 'Unknown';
            const birdsTreated = vaccination.birdsTreated || vaccination.birdsCount || 'N/A';
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                ...
                                <div class="row mb-2">
                                    ...
                                    <div class="col-6">${vaccination.flockAge || 'N/A'} days</div>
                                </div>
                                <div class="row mb-2">
                                    <div class="col-6"><strong>Vaccine:</strong></div>
                                    <div class="col-6">${vaccineName}</div>
                                </div>
                                <div class="row mb-2">
                                    <div class="col-6"><strong>Method:</strong></div>
                                    <div class="col-6">${methodText}</div>
                                </div>
                                <div class="row mb-2">
                                ...
                                <div class="row mb-2">
                                    ...
                                    <div class="col-6">${birdsTreated}</div>
                                </div>
                                <div class="row mb-2">
            ...
            console.error('Error creating vaccination details modal:', error);
            this.showToast('Error displaying vaccination details. Please try again.', 'error');
        }
    }
                ...
                <div class="modal-dialog modal-xl">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <div class="mb-3">
                                <div class="d-flex justify-content-between align-items-center mb-3">
                                    ...
                                    <button class="btn btn-sm btn-success" onclick="window.vaccinationManager.addScheduleItem()">
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="button" class="btn btn-primary" onclick="window.vaccinationManager.saveScheduleFromModal()">
            ...
            <div class="row mb-3 schedule-item" data-index="${index}">
                <div class="col-md-2">
                    ...
                    <input type="number" class="form-control" value="${item.day}" min="1" 
                           onchange="window.vaccinationManager.updateScheduleItem(${index}, 'day', this.value)">
                </div>
                <div class="col-md-2">
                    <label class="form-label">Weeks</label>
                    <input type="number" class="form-control" value="${item.weeks}" min="0" step="0.1"
                           onchange="window.vaccinationManager.updateScheduleItem(${index}, 'weeks', this.value)">
                </div>
                <div class="col-md-3">
                    <label class="form-label">Vaccine</label>
                    <input type="text" class="form-control" value="${item.vaccine}" 
                           onchange="window.vaccinationManager.updateScheduleItem(${index}, 'vaccine', this.value)">
                </div>
                <div class="col-md-3">
                    <label class="form-label">Method</label>
                    <select class="form-select" onchange="window.vaccinationManager.updateScheduleItem(${index}, 'method', this.value)">
                ...
                <div class="col-md-2 d-flex align-items-end">
                    <button class="btn btn-outline-danger btn-sm" onclick="window.vaccinationManager.removeScheduleItem(${index})">
                        <i class="fas fa-trash"></i>
                    </button>
                </div>
            </div>
            ...
            <div class="row mb-3 schedule-item" data-index="${newIndex}">
                <div class="col-md-2">
                    ...
                    <input type="number" class="form-control" value="1" min="1" 
                           onchange="window.vaccinationManager.updateScheduleItem(${newIndex}, 'day', this.value)">
                </div>
                <div class="col-md-2">
                    <label class="form-label">Weeks</label>
                    <input type="number" class="form-control" value="0" min="0" step="0.1"
                           onchange="window.vaccinationManager.updateScheduleItem(${newIndex}, 'weeks', this.value)">
                </div>
                <div class="col-md-3">
                    <label class="form-label">Vaccine</label>
                    <input type="text" class="form-control" value="" placeholder="Enter vaccine name"
                           onchange="window.vaccinationManager.updateScheduleItem(${newIndex}, 'vaccine', this.value)">
                </div>
                <div class="col-md-3">
                    <label class="form-label">Method</label>
                    <select class="form-select" onchange="window.vaccinationManager.updateScheduleItem(${newIndex}, 'method', this.value)">
                        <option value="drinking_water">Drinking Water</option>
                        <option value="injection">Injection</option>
                        <option value="spray">Spray</option>
                        <option value="eye_drop">Eye Drop</option>
                ...
                <div class="col-md-2 d-flex align-items-end">
                    <button class="btn btn-outline-danger btn-sm" onclick="window.vaccinationManager.removeScheduleItem(${newIndex})">
                        <i class="fas fa-trash"></i>
                    </button>
                </div>
            </div>
...
let vaccinationManager;

// Initialize vaccination manager immediately
try {
    vaccinationManager = new VaccinationManager();
    window.vaccinationManager = vaccinationManager;
    console.log('Vaccination manager initialized successfully');
} catch (error) {
    console.error('Error initializing vaccination manager:', error);
}
...
document.addEventListener('DOMContentLoaded', () => {
    document.addEventListener('submit', (e) => {
        if (e.target.id === 'vaccinationForm') {
            window.vaccinationManager.handleVaccinationSubmit(e);
        }
    });
