const settings = {
                        ...
                        <div class="card mb-4">
                            ...
                            <div class="card-body">
                                <div class="row">
                                    ...
                                    <div class="col-md-4">
                                        <h6>Sample Data</h6>
                                        <p class="text-muted small">Re-enable sample data generation for new users</p>
                                        <button type="button" class="btn btn-outline-success" id="enableSampleDataBtn">
                                            <i class="fas fa-seedling me-2"></i>Enable Sample Data
                                        </button>
                                    </div>
                                </div>
                            </div>
    ...
    attachEventListeners() {
        ...
        document.getElementById('enableSampleDataBtn').addEventListener('click', () => {
            this.enableSampleData();
        });
    },
    ...
    async clearAllData() {
        try {
            ...
            // Disable sample data generation so it doesn't regenerate when user logs back in
            localStorage.setItem('sampleDataDisabled', 'true');

            this.showToast('All data cleared successfully. Sample data will not regenerate on next login.', 'success');
            this.loadAppStats();
        } catch (error) {
    ...
    enableSampleData() {
        if (confirm('This will re-enable sample data generation. Sample data will be created the next time you clear all data and restart the app. Continue?')) {
            localStorage.removeItem('sampleDataDisabled');
            this.showToast('Sample data generation has been re-enabled', 'success');
        }
    },
